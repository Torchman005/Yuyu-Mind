...
- **2026-09-15 拟人化 P3-内部独白（"让她有一个不说出口的内心"）**：用户说"先做内部独白吧"，遂推进 [`REALISM-ANALYSIS.md`](REALISM-ANALYSIS.md) P3 里呼声最高、也是**唯一需要新开一条呈现通道**的一项。
  **为什么它有效**：此前角色只有一条通道——**说出口的话**。她说的每一句都是对用户说的，听话的时候心里什么都没有在动。Neuro-sama 的自言自语之所以有"人味"，正因为它不是回答，而是回答之外的一层。
  **三条设计红线（直接决定了实现形态）**：
  ① **不朗读**——独白走**独立事件类型** `EventTypeThought`，与 `EventTypeToken` 严格分离。若混进 token 通道，前端会把它推进 TTS 队列念出来，"内心"立刻退化成"第二段台词"，效果反向；前端 `handleChatEvent` 里也留了明确注释"这里不要调用 `handleStreamToken`"。
  ② **不落库、不进历史**——独白一旦写进 `messages` 表，下一轮就会作为 assistant 历史喂回模型，它会开始"回应自己的心声"，同时污染记忆抽取。它必须是即兴且无痕的。代价是重启不留痕，而这恰好符合语义：**念头本来就不留痕**。
  ③ **偶发**——每轮都说就成了新口癖。做了两层克制：模型侧自评"有没有真的闪过念头"（无则输出 `""`，提示词里明确写"大多数时候空串才是诚实答案"），服务侧再用**冷却 20s + 概率 0.6** 挡一次（`thought.go` 的 `monologueGate`）。
  **一个不显然的实现难点（本轮最容易做错的地方）**：`dialogStreamParser` 为了"逐句流式下发"，会把**第一个完整台词对象之前的所有文本丢弃**——正是靠这个技巧，包装前缀 `{"dialog":[` 才不干扰逐项解析。而 `thought` 字段恰好就在那个前缀里，**照直写会 100% 丢失，且只在流式路径丢、单 chunk 不丢**，属于最难复现的一类 bug。解法是在 `drain()` **之前**用 `captureThought()` 从 `accumulated` 增量抽取：`extractThoughtField()` 按字节扫描值区间，遇到收尾引号才算完整，自己处理 `\"` `\\` `\n` `\uXXXX` 以及"转义被 chunk 切断"（未到齐就等下一片，不猜半个字符）；非法转义按字面反斜杠保留，与既有 `fixInvalidJSONEscapes` 口径一致。
  **清洗（`normalizeThought`）**：压平成单行 → 剥"字段名前缀"（`thought:`「心声：」）与**成对的外层**括号/引号（模型极爱把心里话写成「（…）」）→ 挡掉占位值（`none`/`null`/`无`）与**纯标点**（`""`「……」——模型用它们填"可选但不想写"的字段，用 `hasThoughtContent` 判定有无文字/数字，CJK 由 `unicode.IsLetter` 覆盖）→ 长度护栏 2–34 字（超长按字符截断补省略号）。刻意只剥**成对**的外层括号，因此「（笑）嗯…刚才那个」里的语气补充不会被动。清洗被断言为**幂等**，避免前后端二次处理产生差异。
  **提示词的写法**：给 `thought` 单开一段（而非塞进字段列表一行），因为它太容易被理解成"再来一句台词"。关键几条：大多数时候给空串；第一人称、**可以不礼貌不周到**（"心里话不必客气，念头与台词之间的落差才是重点"）；**不得称呼「主人」**、不得向用户提问；**不得复述/总结/预告 `speech`**，且"可以完全跑题——真人的心思会走神"。
  **呈现层**：`RoomView` 新增「心声」浮层，位置固定在**舞台底部中央偏上（`bottom:110px`）**而不是贴着 footer——状态岛占的是 footer 上方 `left:16px`、高约 32px 的一条带，同高会在窄窗口被横向压到，抬高一层就彻底避开；上方又不会与台词气泡争。**视觉语言刻意与台词气泡做成两套**：说出口的话＝实心白卡+粗体居中+投影（存在感强）；心里的话＝**虚线幽灵+斜体弱色+无投影**（存在感弱）。这个层级差本身就是"她没说出来"的表达——做成同一个气泡，用户会以为她又说了一句话。8 秒后自行淡出并**轻微上浮**（像念头自己散去，而不是被擦掉）；`prefers-reduced-motion` 下停用出入场位移，文字仍照常出现与移除（比让它彻底不出现更尊重"弱动态"的本意）。
  **一个易错的同步点**：CSS 动画总时长必须与 `RoomView.THOUGHT_HOLD_MS` 一致（`8000ms − 640ms` 的淡出延迟 ↔ 动画参数），两边都写了注释互相指认，避免"文字已移除但动画没走完"或"淡出后留一块空白"。
  **验证**：新增 `thought_test.go` 共 **11 项**（其中 `normalizeThought` 22 个用例、`shouldSurfaceThought` 12 个边界、`extractThoughtField` 12 类转义）——清洗（嵌套括号／行内括号必须保留／多行压平／占位值／纯标点／截断／幂等）、门控的冷却/记录/会话隔离/关闭时不占冷却、流式解析三种时序（前缀被丢弃前捕获／单 chunk／独白后置／无独白／占位值），以及**端到端** `streamReply` 三条：独白事件确实下发且**排在台词之前**、独白**不出现在任何 token**（否则会被念出来）、独白**不进消息表**；另加一条"开关关闭时一句都不出现"作为**配置项必须有消费点**的回归（本项目出现过 `AllowTypoSimulation` 那样的死配置），以及一条**锁住复用陷阱**的回归（断言 `postprocessReply` 确实会删空「（…）」形式的独白，因此两条清洗路径不能合并）。全量：`go build ./...`、`go vet ./internal/...`、`go test ./internal/...`（13 包全绿）、`tsc --noEmit`、`npm run build`、插件 9+4 项全绿。
  **明确未做（保持范围）**：主动搭话（`proactive.go`）走的是另一套**纯文本提示词**、不是结构化 JSON，要让它的独白先于发言出现必须改造那条链路；本轮不动，已记入 `REALISM-ANALYSIS.md` 的 P3 清单。
