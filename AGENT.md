...
- [x] 2026-09-15 **拟人化 P3 内部独白（"让她有一个不说出口的内心"）**（依据 [`docs/REALISM-ANALYSIS.md`](docs/REALISM-ANALYSIS.md) P3 可选实验）。**问题**：此前角色只有一条通道——**说出口的话**，她说的每一句都是对用户说的，听话的时候心里没有任何东西在动；这正是"像应答机而不像在场者"的最后一层差距（Neuro-sama 的自言自语之所以有"人味"，是因为它不是回答，而是回答之外的一层）。**三条设计红线（直接决定实现形态）**：① **不朗读**——独白走**独立事件类型** `ChatEvent(EventTypeThought)`，与 `EventTypeToken` 严格分离；若混进 token 通道，前端会把它推进 TTS 队列念出来，"内心"立刻退化成"第二段台词"，效果反向。② **不落库、不进历史**——独白一旦写进 `messages` 表，下一轮就会作为 assistant 历史喂回模型，它会开始"回应自己的心声"并污染记忆抽取；必须是即兴且无痕的（重启不留痕恰好符合"念头本来就不留痕"的语义）。③ **偶发**——模型侧自评"有没有真的闪过念头"（无则输出 `""`），服务侧再用**冷却 20s × 概率 0.6** 二次克制，避免变成新口癖。**实现**：新增 `internal/chat/thought.go`（`normalizeThought` 清洗 + `shouldSurfaceThought`/`monologueGate` 门控 + `Service.allowMonologue`）；`dialog_parser.go` 新增 `extractThoughtField` 与 `captureThought`/`takeThought`；`stream_reply.go` 新增 `emitThought` 并在**台词之前**调用；`agents.go` Replyer 增加 `{"thought": "...", "dialog": [...]}` 输出契约；`config.go` 新增 `ChatConfig.AllowInnerMonologue`（json `allow_inner_monologue`，默认 true）；前端 `RoomView` 新增「心声」浮层（`App.tsx` 仅接管事件与状态，**明确不调用 `handleStreamToken`**）。**本轮最关键的实现难点**：`dialogStreamParser` 为了逐句流式下发，会把**第一个完整台词对象之前的所有文本丢弃**（正是靠这个技巧包装前缀 `{"dialog":[` 才不干扰逐项解析），而 `thought` 恰好就在那个前缀里——**照直写会 100% 丢失，且只在流式路径丢、单 chunk 不丢**，属于最难复现的一类 bug。解法是在 `drain()` **之前**从 `accumulated` 增量抽取：按字节扫描值区间、遇收尾引号才算完整，自行处理 `\"` `\\` `\n` `\uXXXX` 与"转义被 chunk 切断"（未到齐就等下一片），非法转义按字面反斜杠保留（与既有 `fixInvalidJSONEscapes` 同口径）。**清洗规则**：压平单行 → 剥字段名前缀与**成对的外层**括号/引号（模型极爱写成「（…）」，但刻意只剥成对的外层，所以「（笑）嗯…刚才那个」的语气补充不受影响）→ 挡占位值（`none`/`null`/`无`）与纯标点（`""`「……」）→ 长度护栏 2–34 字（按字符截断补省略号）；清洗被断言**幂等**。**呈现设计（`DESIGN.md` 已写入规范）**：位置固定在舞台底部中央偏上（`bottom:110px`，避开状态岛所占的 footer 上方带，也不与顶部台词气泡争）；视觉语言**刻意与台词气泡做成两套**——说出口的话＝实心白卡+粗体居中+投影，心里的话＝**虚线幽灵+斜体弱色+无投影**+`⋯` 引导符，这个层级差本身就是"她没说出来"的表达；8 秒后自行淡出并轻微上浮，`prefers-reduced-motion` 下停用位移但文字照常出现与移除。**验证**：`go build ./...` + `go vet ./internal/...` + `go test ./internal/...`（13 包全绿）+ `tsc --noEmit` + `npm run build` + 插件 9/4 项；新增 `thought_test.go` **11 项**（含 `normalizeThought` 22 用例与幂等断言、`shouldSurfaceThought` 12 边界含"冷却优先于概率"、门控冷却/记录/会话隔离/关闭不占冷却、`extractThoughtField` 12 类转义、流式解析四种时序、端到端三条：独白**排在台词之前** / **不出现在任何 token** / **不进消息表**，另加"开关关闭时一句都不出现"作为**配置项必须有消费点**的回归，以及一条锁住**清洗器复用陷阱**的回归——断言 `postprocessReply` 确实会删空「（…）」形式的独白，因此两条清洗路径不可合并）。**未做**：主动搭话（`proactive.go`，纯文本提示词而非结构化 JSON）尚未接入独白通道，已记入 `REALISM-ANALYSIS.md`。
